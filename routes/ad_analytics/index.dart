...
Future<Response> onRequest(RequestContext context) async {
  ...
  try {
    ...
    if (request.method == HttpMethod.get) {
      final result = await promotionService.listAdAnalytics(
        idToken: idToken,
        artisanId: request.uri.queryParameters['artisanId'],
        postId: request.uri.queryParameters['postId'],
        days: int.tryParse(request.uri.queryParameters['days'] ?? '') ?? 30,
    ...
    final created = await promotionService.createAdAnalyticsEvent(
      idToken: idToken,
      payload: body,
    );
