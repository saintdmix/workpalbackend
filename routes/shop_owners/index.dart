...
Future<Response> onRequest(RequestContext context) async {
  ...
  try {
    ...
    if (request.method == HttpMethod.get) {
      final result = await commerceService.listShopOwners(
        idToken: idToken,
        limit: int.tryParse(request.uri.queryParameters['limit'] ?? '') ?? 50,
        pageToken: request.uri.queryParameters['pageToken'],
        zoneId: request.uri.queryParameters['zoneId'],
        search: request.uri.queryParameters['search'],
        verified: _parseBool(request.uri.queryParameters['verified']),
      );
    ...
    final ownerId =
        '${body['ownerId'] ?? body['id'] ?? body['userId'] ?? ''}'.trim();
    if (ownerId.isEmpty) {
      throw ApiException.badRequest('ownerId is required.');
    }
    final result = await commerceService.upsertShopOwner(
      idToken: idToken,
      ownerId: ownerId,
      payload: body,
    );
