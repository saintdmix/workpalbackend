...
Future<Response> onRequest(RequestContext context, String uid) async {
  ...
  try {
    ...
    if (request.method == HttpMethod.get) {
      final result = await nriLegacyService.getUserCart(
        idToken: idToken,
        uid: uid,
      );
    ...
    if (request.method == HttpMethod.delete) {
      await nriLegacyService.deleteUserCart(idToken: idToken, uid: uid);
    ...
    final result = request.method == HttpMethod.put
        ? await nriLegacyService.putUserCart(
            idToken: idToken,
            uid: uid,
            payload: body,
          )
        : await nriLegacyService.patchUserCart(
            idToken: idToken,
            uid: uid,
            payload: body,
          );
